...
public final class CacheManager {
   ...
   */
  //负责扫描命名空间和活跃的缓存命令，以确定需要缓存或删除缓存的数据块，向datanode分配缓存任务
  ...
  private static long validateExpiryTime(CacheDirectiveInfo info,
    ...
    //如果缓存命令没有设置过期时间，默认返回当前时间加上缓存池的过期时间限制
    ...
    if (expiry.isRelative()) {//如果指定的是相对时间
  ...
  private CacheDirectiveStats computeNeeded(String path, short replication) {
    ...
    } else if (node.isDirectory()) {//如果缓存目录，则缓存的是目录下的文件
