...
public abstract class Storage extends StorageInfo {
  ...
  public enum StorageState {
    NON_EXISTENT,//存储不存在
    NOT_FORMATTED,//未格式化
    COMPLETE_UPGRADE,//升价完成
    RECOVER_UPGRADE,//恢复升级
    COMPLETE_FINALIZE,//完成升级
  ...
  //存储该storage所管理的所有StorageDirectory
  ...
  public static class StorageDirectory implements FormatConfirmable {
    ...
    final boolean isShared;//指定当前目录是否是共享的
    ...
    public StorageState analyzeStorage(StartupOption startOpt, Storage storage)
      ...
      try { // check that storage exists
        ...
        if (!root.exists()) {
          // storage directory does not exist
        	//如果不存在，并且启动项不是format和hotswap，则为NON_EXISTENT状态
          if (startOpt != StartupOption.FORMAT &&
              startOpt != StartupOption.HOTSWAP) {
            LOG.warn("Storage directory " + rootPath + " does not exist");
            return StorageState.NON_EXISTENT;
          }
          //如果是format或hotswap，则创建该目录
        ...
        // or is inaccessible
        //如果root不是目录，也是NON_EXISTENT状态
        if (!root.isDirectory()) {
          LOG.warn(rootPath + "is not a directory");
          return StorageState.NON_EXISTENT;
        }
        //如果不可写，也是NON_EXISTENT状态
        if (!FileUtil.canWrite(root)) {
          LOG.warn("Cannot access storage directory " + rootPath);
          return StorageState.NON_EXISTENT;
        }
      } catch(SecurityException ex) {//权限问题异常
      ...
      if (!(hasPreviousTmp || hasRemovedTmp
        ...
        //没有tmp文件，也没有current文件，但是有previous文件
        if (hasPrevious)
          throw new InconsistentFSStateException(root,
                              "version file in current directory is missing.");
        //没tmp ，没current，没previous
        return StorageState.NOT_FORMATTED;
      }
    ...
    FileLock tryLock() throws IOException {
      ...
      //如果文件存在，调用他的deleteOnExit，在退出是删除文件
      if (!lockF.exists()) {
        lockF.deleteOnExit();
