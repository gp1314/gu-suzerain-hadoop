...
public class FSDirectory implements Closeable {
  ...
  INodesInPath addFile(INodesInPath existing, String localName, PermissionStatus
    ...
    //将inodefile对象装换成FileUnderConstructionFeature的正在构建状态
    ...
    try {
      ...
      BlockInfoContiguousUnderConstruction blockInfo =
        new BlockInfoContiguousUnderConstruction(
            ...
            targets);
      //使用blockmanger添加block
      getBlockManager().addBlockCollection(blockInfo, fileINode);
      //将block与文件对应的INode对象关联
  ...
  boolean unprotectedRemoveBlock(String path, INodesInPath iip,
    ...
    //将block从blocksMap以及corruptReplicas中移除
  ...
  public INodesInPath addLastINode(INodesInPath existing, INode inode,
    ...
    //不能在 /.reserved目录下创建文件或目录
    //这可能是在升级期间加载editlog/fsimage时创建的，因为/.reserved下是旧版本中的一个有效名称。
    //当用户试图创建文件或目录/.reserved时，也可能会调用它。
   ...
   * 2.) INode length is truncated now 鈥� new clients can only read up to
  ...
  static String resolvePath(String src, byte[][] pathComponents,
    ...
    //如果是.inodes的文件，将其中的inode解析到外面
    //如,/.reserved/.inodes/324/a/b其中324为inode的编号，表示一个目录，假设为test
    //解析为/test/a/b
    if (Arrays.equals(DOT_INODES, pathComponents[2])) {
      ...
      } else {//将形如/.reserved/raw/ezone/a解析为/ezone/a
