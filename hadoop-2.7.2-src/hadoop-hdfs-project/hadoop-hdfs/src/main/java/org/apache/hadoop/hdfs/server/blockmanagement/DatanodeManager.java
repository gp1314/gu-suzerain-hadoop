...
public class DatanodeManager {
   ...
   */
  //维护storageid->DatanodeDescriptor映射关系
  ...
  public void registerDatanode(DatanodeRegistration nodeReg)
    ...
    try {
      ...
      if (nodeS != null) {
        ...
        try {
          ...
          if(this.rejectUnresolvedTopologyDN) {
        	  //拒绝就是resolveNetworkLocation直接抛出异常
      ...
      try {
        ...
        //添加到datnodeMap以及host2DatanodeMap中
  ...
  public void refreshNodes(final Configuration conf) throws IOException {
	  //加载include文件与exclude文件至hostFileManager
    refreshHostsReader(conf);
    namesystem.writeLock();
    try {
    	//根据include文件与exclude文件改变节点状态
      refreshDatanodes();
      countSoftwareVersions();
    } finally {
      namesystem.writeUnlock();
  ...
  private void refreshDatanodes() {
    for(DatanodeDescriptor node : datanodeMap.values()) {
      // Check if not include.
    	//如果不在include文件中，则设置其不能与namenode通信
      if (!hostFileManager.isIncluded(node)) {
        node.setDisallowed(true); // case 2.
      } else {//如果在include中，在exclude中，则设置该节点退役
        if (hostFileManager.isExcluded(node)) {
          decomManager.startDecommission(node); // case 3.
        } else {//如果在include中，不在exclude中，则设置该节点停止退役，即从退役状态转换为正常状态
  ...
  public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
    ...
    synchronized (heartbeatManager) {
      synchronized (datanodeMap) {
        ...
        //检查当前datanode能否接入namenode
        if (nodeinfo != null && nodeinfo.isDisallowed()) {
          setDatanodeDead(nodeinfo);
          throw new DisallowedDatanodeException(nodeinfo);
        }
        //确认当前节点已经在namenode中注册，否则发送注册命令
        ...
        if (blocks != null) {
          ...
          for (BlockInfoContiguousUnderConstruction b : blocks) {
            ...
            // 如果在排除stale节点后只有一个节点可用于恢复，那么选择所有节点进行恢复，让主节点处理失败
