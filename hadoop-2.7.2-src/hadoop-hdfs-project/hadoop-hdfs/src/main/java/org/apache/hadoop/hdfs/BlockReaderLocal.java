...
class BlockReaderLocal implements BlockReader {
   ...
   */
  //ShortCircuitReplica类，保存了用来执行性短路读取的文件描述符
  //client和datanode共享内存中记录的当前副本信息的slot对象,以及数据块在内存中的映射mmapData
    ...
    try {
      ...
      try {
        ...
        if (canSkipChecksum && zeroReadaheadRequested) {
          ...
          //需要校验，或者开启了预读取时，调用readWithBounceBuffer
  ...
  private synchronized int readWithBounceBuffer(ByteBuffer buf,
    ...
    int total = 0;
    //将databuf缓存区中的数据拉取到buf中，然后返回读取的字节数
    //使读取位置在chunk的边界
    ...
    do {
      if (buf.isDirect() && (buf.remaining() >= maxReadaheadLength)
        ...
        //如果读取的数据小于maxReadaheadLength，表示读取完成
    	...
    	//将数据读入databuf中，将校验和数据读入checksumbuf缓冲区中
        if (fillDataBuf(canSkipChecksum)) {
          done = true;
        }
        //剩余不足maxReadaheadLength大小的数据从databuf缓存中读取
