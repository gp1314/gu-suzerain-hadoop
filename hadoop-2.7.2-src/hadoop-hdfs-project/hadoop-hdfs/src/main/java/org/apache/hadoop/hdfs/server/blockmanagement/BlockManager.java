...
public class BlockManager {
  ...
  //保存损坏的数据块副本，来源于ClientProtocol.reportBadBlocks() 和 DatanodeProtocol.reportBadBlocks()
  ...
  public void activate(Configuration conf) {
	//遍历所有的pendingReplications集合中保存的所有数据块复制任务，将超过指定时间没有确认的复制请求加入超时队列中
    pendingReplications.start();
    datanodeManager.activate(conf);
    //执行副本复制、删除以及扫描推迟操作的数据块执行多余副本操作
  ...
  private BlockInfoContiguous completeBlock(final BlockCollection bc,
    ...
    //将blocksMap以及datanode中的映射关系都替换为BlockInfo
  ...
  public LocatedBlock convertLastBlockToUnderConstruction(
    ...
    //如果最后一个块写满了，或者这个文件时一个刚刚创建的文件，则返回null
    if(oldBlock == null ||
    ...
    namesystem.adjustSafeModeBlockTotals(
        ...
        -1);
    //构造LocatedBlock返回
    //computeContentSummary方法应该继承inode相关方法
  ...
  void removeBlocksAssociatedTo(final DatanodeDescriptor node) {
    ...
    //从nanmenode standby状态的pendingDNMessages队列中，移除相关节点
  ...
  private void markBlockAsCorrupt(BlockToMarkCorrupt b,
    ...
    //如果该数据块不属于任何文件，则直接删除
    //例如，该数据块副本对应的hdfs文件已经被删除，但数据节点上的副本还未删除
    if (bc == null) {
  ...
  int computeReplicationWork(int blocksToProcess) {
    ...
    try {
      // Choose the blocks to be replicated
    	//从优先队列中选出blocksToProcess数量的需要复制的数据块
      ...
      synchronized (neededReplications) {
        for (int priority = 0; priority < blocksToReplicate.size(); priority++) {
          for (Block block : blocksToReplicate.get(priority)) {
            ...
            // not included in the numReplicas.liveReplicas() count
            // liveReplicaNodes包含READ_ONLY_SHARED副本，numReplicas.liveReplicas()不包含
            assert liveReplicaNodes.size() >= numReplicas.liveReplicas();

            // do not schedule more if enough replicas is already pending
            //当前有效副本数 = 已经备份数目 + 正在备份数目
            numEffectiveReplicas = numReplicas.liveReplicas() +
                                    pendingReplications.getNumReplicas(block);
            //如果副本数已经足够，则不用进行备份操作，从需要复制的队列中移除
            ...
            //在工作队里中添加备份任务，任务会在下次心跳时带到datanode
    ...
    try {
      for(ReplicationWork rw : work){
        ...
        synchronized (neededReplications) {
          ...
          //如果是机架数不足导致的复制任务，并且重新选择的机架相同，则跳过该复制任务
          if ( (numReplicas.liveReplicas() >= requiredReplication) &&
               (!blockHasEnoughRacks(block)) ) {
          ...
          //将副本添加到datanode的备份队里中，在下一次心跳时，发出备份指令
   ...
   DatanodeDescriptor chooseSourceDatanode(Block block,
    ...
    for(DatanodeStorageInfo storage : blocksMap.getStorages(block)) {
      ...
      if ((nodesCorrupt != null) && (nodesCorrupt.contains(node)))
        corrupt += countableReplica;//统计该块损坏的节点
      else if (node.isDecommissionInProgress() || node.isDecommissioned())
        decommissioned += countableReplica;//统计该块对应的退役的节点
      else if (excessBlocks != null && excessBlocks.contains(block)) {
        excess += countableReplica;//统计该块对应的多余副本节点
      } else {//该块有效数据节点
      ...
      {
    	  //如果优先级不是最高，节点不是退役节点，节点要执行的复制块大于maxReplicationStreams（2）
    	  //则跳过该节点
        continue; // already reached replication limit
      }
      if (node.getNumberOfBlocksToBeReplicated() >= replicationStreamsHardLimit)
      {//节点要执行的复制块大于replicationStreamsHardLimit直接跳过
        continue;
      }
      ...
      //这将随机切换到另一个节点，以防止确定性地选择同一个节点，即使该节点在以前的迭代中未能复制该块。
  ...
  private Collection<Block> processReport(
    ...
    //数据块处于构建状态，构造ReplicateUnderConstruction对象
    //将该对象添加到数据块对应的BlockInfoUnderConstruction对象的replicas队列中
    for (StatefulBlockInfo b : toUC) { 
      addStoredBlockUnderConstruction(b, storageInfo);
    }
    //副本在namenode中的DatanodeStorageInfo对象上存储在，但datanode汇报时没有该副本
    for (Block b : toRemove) {
      removeStoredBlock(b, node);
    }
    int numBlocksLogged = 0;
    //时间戳，文件长度都一致，将副本添加到namenode内存中
    ...
    //blocksMap中没有上报的副本的信息，将其添加到Invalidates队列中
    for (Block b : toInvalidate) {
      addToInvalidates(b, node);
    }
    //时间戳，文件长度不一致，将其添加到corrupReplicas队列中
  ...
  private void processFirstBlockReport(
    ...
    for (BlockReportReplica iblk : report) {
      ...
      //如果当前namenode是standby，那么可能出现块汇报时standby没有完全更新editlog的情况
      //这时需要等待standby更新到最新的editlog
      ...
      if (c != null) {
        ...
        } else {//将损坏节点放入invalidateBlocks队列中
      ...
      //如果该块处于构建状态，那么将这个副本加入对应的构建中副本队列
      ...
      //如果该副本处于FINALIZED状态，则将其加入namenode中
  ...
  private void reportDiff(DatanodeStorageInfo storageInfo, 
	...
	//在blockList设置一个分隔符，来划分汇报的块和没有汇报的块
  ...
  private BlockInfoContiguous processReportedBlock(
    ...
    if(storedBlock == null) {
      ...
      //daanode上报的块不在blocksMap中，将这个副本添加到toInvalidate队列
    ...
    //对比namenode中的BlockInfoContiguous状态与上报的Block状态对比确定损坏的节点
    BlockToMarkCorrupt c = checkReplicaCorrupt(
    ...
    //对比namenode中的BlockInfoContiguous状态与上报的Block状态确定是否是正在构建块
    if (isBlockUnderConstruction(storedBlock, ucState, reportedState)) {
    ...
    //如果上报副本有效，在BlockInfoContiguous中没有该副本或者该副本为损坏副本，加入toAdd队列
    if (reportedState == ReplicaState.FINALIZED
  ...
  private BlockToMarkCorrupt checkReplicaCorrupt(
    ...
    case RWR:
      if (!storedBlock.isComplete()) {
        ...
        //汇报的RBW状态，而namenode中的block确实COMPLETE状态
        //可能是汇报有延迟于数据传输管道的关闭,则忽略该情况
  ...
  throws IOException {
    ...
    //如果不在安全模式或namenode在standby，则进行较慢的加入副本操作
    if (!namesystem.isInStartupSafeMode() 
    ...
    // just add it
    //在namenode内存中添加这个数据块副本的信息
    AddBlockResult result = storageInfo.addBlock(storedBlock);

    // Now check for completion of blocks and safe block count
    //如果当前数据块处于commited状态，并且数据块满足最小副本要求，调用completeBlock将数据块状态更新为COMPLETE
  ...
  throws IOException {
    ...
    if (result == AddBlockResult.ADDED) {
      ...
      // corrupt block list.
    	//如果是多余的添加的块，并且损坏原因是时间戳失效，则将块从损坏队里中移除
    ...
    //如果该副本为COMMITTED状态，则将namenode中保存的当前数据块的状态转化为正常状态
    //就是将inode和blocksMap中保存的BlockInfoUnderConstruction转换为BlockInfo
    if(storedBlock.getBlockUCState() == BlockUCState.COMMITTED &&
  ...
  private MisReplicationResult processMisReplicatedBlock(BlockInfoContiguous block) {
    ...
    //如果有多余副本，需要判断是否有任何一个datanode处于stale状态。
    //如果有，就不能直接删除任何一个副本，因为有些副本已经被删除，从而导致丢数据。
    //如果没有，则将选择多余的副本放入多余副本队列
  ...
  private void processOverReplicatedBlock(final Block block,
    ...
    for(DatanodeStorageInfo storage : blocksMap.getStorages(block, State.NORMAL)) {
      ...
      //如果有任何一个datanode在stale状态，将该block加入到延迟处理队列
      if (storage.areBlockContentsStale()) {
      ...
      //挑选出该block不在多余副本队列中，不在退役状态的datanode
  ...
  /*
   * 该函数用于删除多余副本
   * 该算法首先从包含多个块副本的机架上的节点中选择一个空间最少的节点，删除该副本不会移除一个机架
   * 如果没有这样的节点可用，那么选择一个空间最少的节点
   * */
  private void chooseExcessReplicates(final Collection<DatanodeStorageInfo> nonExcess, 
    ...
    //将候选的block分为多余一个副本的机架和只有一个副本的机架
    ...
    while (nonExcess.size() - replication > 0) {
      ...
      //重新修正moreThanOne,exactlyOne队列
      replicator.adjustSetsWithChosenReplica(rackMap, moreThanOne,
          exactlyOne, cur);
      //将选择的datanode和block加入多余副本队列
  ...
  public void removeStoredBlock(Block block, DatanodeDescriptor node) {
    ...
    assert (namesystem.hasWriteLock());
    {
    	//删除blocksMap以及datanode中的副本,以及block（如果不属于任何文件）
      ...
      // be-replicated list.
      //
      //这次删除操作可能是数据节点发生错误，但是该数据块依然有效
      //在这种情况下数据块的副本数量可能不足，更新neededReplication队列
      ...
      // in "excess" there.
      //
      //已经从数据节点删除了副本，更新excessReplicateMap队列
      ...
      //已经从数据节点上删除了副本，更新corruptReplicas队列
      corruptReplicas.removeFromCorruptReplicasMap(block, node);
    }
  }
  ...
  void addBlock(DatanodeStorageInfo storageInfo, Block block, String delHint)
    ...
    //获取delHintNode，希望从delHintNode中删除该数据块
  ...
  private void processAndHandleReportedBlock(
    ...
    //这个block只能在一个todo队列，如果都不在，则datanode中已经有了该block
  ...
  private void updateNeededReplications(final Block block,
    ...
    try {
      ...
      } else {//副本系数足够，从neededReplications删除该数据块
  ...
  public BlockInfoContiguous addBlockCollection(BlockInfoContiguous block,
      BlockCollection bc) {
	  //将blockinfo对象添加到blockmanager的blocksMap中
 ...
 public String getCorruptReason(Block block, DatanodeDescriptor node) {
      ...
      while (namesystem.isRunning()) {
        try {
          // Process replication work only when active NN is out of safe mode.
        	//namenode脱离安全模式时才能工作
          if (namesystem.isPopulatingReplQueues()) {
        	//计算需要复制的数据块执行复制操作，计算需要删除的复制块执行删除操作
            computeDatanodeWork();
            //扫描超时复制的数据块
  ...
  int computeDatanodeWork() {
    ...
    //计算出进行复制操作的数据块数量,blocksReplWorkMultiplier默认为2
    final int blocksToProcess = numlive
        * this.blocksReplWorkMultiplier;
    //计算出进行删除操作的datanode数量，blocksInvalidateWorkPct默认为0.32
