...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
  ...
  private int[] getMaxNodesPerRack(int numOfChosen, int numOfReplicas) {
    ...
    //numOfChosen代表已经存在的副本个数，numOfReplicas代表需要备份的副本数
    int totalNumOfReplicas = numOfChosen + numOfReplicas;
    //如果总备份的副本数大于数据节点数，则设置总副本数为节点个数
  ...
  protected DatanodeStorageInfo chooseLocalStorage(Node localMachine,
    ...
    //这里要注意DatanodeDescriptor和DatanodeStorageInfo的区别
    //引入DatanodeStorageInfo实现异步存储，副本的最小存储单元由DatanodeDescriptor变为了DatanodeStorageInfo
    //DatanodeStorageInfo代表datanode上的一块storage，一个datanode上有多个异构的storage存储副本
  ...
  public DatanodeStorageInfo chooseReplicaToDelete(BlockCollection bc,
    ...
    //如果所有的心跳都在可容忍的心跳间隔内，首先选择心跳间隔最长的，其次选择剩余空间最少的
