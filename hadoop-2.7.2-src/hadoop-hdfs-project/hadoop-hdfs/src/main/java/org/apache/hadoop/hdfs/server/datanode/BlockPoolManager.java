...
class BlockPoolManager {
  ...
  private void doRefreshNamenodes(
    ...
    //需要更新BPOfferService的列表
    Set<String> toRefresh = Sets.newLinkedHashSet();
    //需要添加BPOfferService的列表
    Set<String> toAdd = Sets.newLinkedHashSet();
    //需要删除BPOfferService的列表
    Set<String> toRemove;
    ...
    synchronized (this) {
      ...
      // need to be removed.
      //配置列表中不存在，但内存集合中存在的，将其添加到需要删除的列表中
    ...
    //这个过程不在同步块中执行是因为整个关闭操作是在actor工作线程的 cleanUp()中完成的
    //对于BPOfferService在BlockPoolManager中的映射关系也是在cleanUp()中移除的
    if (!toRemove.isEmpty()) {
