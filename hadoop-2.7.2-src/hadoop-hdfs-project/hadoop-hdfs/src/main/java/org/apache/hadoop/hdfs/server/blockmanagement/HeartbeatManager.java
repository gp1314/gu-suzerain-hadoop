...
class HeartbeatManager implements DatanodeStatistics {
  ...
  synchronized void updateHeartbeat(final DatanodeDescriptor node,
	  ...
	  //先删除之前的node的状态指标，更新完node指标后，在加上node新的状态指标
    stats.subtract(node);
   ...
   */
  void heartbeatCheck() {
	  //在移除死数据时，确保在同步部分中，一次只标记一个DataNode故障。
	  //否则，级联效应会导致更多的数据节点被声明为死。
    ...
    while (!allAlive) {
      ...
      //标志第一datanode正常但存在损坏的storage
      DatanodeStorageInfo failedStorage = null;

      // check the number of stale nodes
      int numOfStaleNodes = 0;
      int numOfStaleStorages = 0;
      //timeout = 2 * heartbeatRecheckInterval + 10 * 1000 * heartbeatIntervalSeconds;
      synchronized(this) {
