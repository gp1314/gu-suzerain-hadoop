 ...
 */
/*
 * 用于保存等待删除的副本集合，需要删除的副本来自于corruptReplicas和excessReplicateMap两个集合
 * 保存DatanodeInfo与block集合的映射关系，该数据由BlockManager的ReplicationMonitor线程定期执行删除操作
 * */
...
class InvalidateBlocks {
  ...
  synchronized List<Block> invalidateWork(final DatanodeDescriptor dn) {
    ...
    //在启动namenode后经过pendingPeriodInMs的延迟才能执行删除任务
    if (delay > 0) {
