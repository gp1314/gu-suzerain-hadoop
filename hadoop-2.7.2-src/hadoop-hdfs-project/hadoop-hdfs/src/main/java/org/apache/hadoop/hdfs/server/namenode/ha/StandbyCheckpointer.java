...
public class StandbyCheckpointer {
  ...
  private void doCheckpoint() throws InterruptedException, IOException {
    ...
    try {
      ...
      //获取当前standbynamenode上保存的最新的fsimage文件
      FSImage img = namesystem.getFSImage();
      //获取fsimage中保存的txid，也就是上一次进行检查点操作是保存的txid
      long prevCheckpointTxId = img.getStorage().getMostRecentCheckpointTxId();
      //获取当前命名空间的最新txid，也就是收到的editlog的最新txid
    ...
    // See HDFS-4816
    //构造一个线程，通过http将fsimage文件上传到active namendoe
    ...
    private void doWork() {
      ...
      while (shouldRun) {
        ...
        try {
          ...
          //获取最后一次往JournalNode写入的txid和最近一次做检查点的txid的差值
