 ...
 * <ol>
 *   <li>{@link #QUEUE_HIGHEST_PRIORITY}(优先级0): the blocks that must be replicated
 ...
 *   remain fails.</li>
 *   <li>{@link #QUEUE_VERY_UNDER_REPLICATED}(优先级1): blocks that are very
 ...
 *   <li>{@link #QUEUE_UNDER_REPLICATED}(优先级2): blocks that are also under
 ...
 *   queue.</li>
 *   <li>{@link #QUEUE_REPLICAS_BADLY_DISTRIBUTED}(优先级3): there are as least as
 ...
 *   <li>{@link #QUEUE_WITH_CORRUPT_BLOCKS}(优先级4): This is for blocks that are corrupt
...
class UnderReplicatedBlocks implements Iterable<Block> {
  ...
  public synchronized List<List<Block>> chooseUnderReplicatedBlocks(
    ...
    // initialize data structure for the return value
	  //初始化返回值列表，保存每个优先级队列中取出的数据块
    List<List<Block>> blocksToReplicate = new ArrayList<List<Block>>(LEVEL);
    for (int i = 0; i < LEVEL; i++) {
      blocksToReplicate.add(new ArrayList<Block>());
    }

    if (size() == 0) { // There are no blocks to collect.没有保存的任何待复制块
    ...
    for (int priority = 0; priority < LEVEL; priority++) { 
      ...
      if (blockCount == blocksToProcess) {
        break;  // break if already expected blocks are obtained 如果读取了足够的数量，退出循环
      }
      ...
      if (!neededReplicationsIterator.hasNext()
        ...
        // recently added blocks in any list.
    	  //将所有优先级队列的读取偏移量置为0，因为最近没有新添加的待复制副本
        for (int i = 0; i < LEVEL; i++) {
          priorityToReplIdx.put(i, 0);
        }
        break;
   ...
   */
  //remove删除数据元素操作会造成priorityToReplIdx字段记录的偏移量错误
  //需要调用该方法更新偏移量
