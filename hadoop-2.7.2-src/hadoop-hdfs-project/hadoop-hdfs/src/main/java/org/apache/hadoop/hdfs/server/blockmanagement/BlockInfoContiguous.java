...
public class BlockInfoContiguous extends Block
  ...
  //记录了该hdfs文件的INode对象引用，表示该数据块属于哪个HDFS文件
  ...
  boolean removeStorage(DatanodeStorageInfo storage) {
    ...
    //用最后一个副本替换dnIndex指定的一个副本的空间，包括链表的前后继block
  ...
  BlockInfoContiguous listRemove(BlockInfoContiguous head,
    ...
    this.setPrevious(dnIndex, null);
    //这里一定关注prev.findStorageInfo(storage)
    //他是指定找到传入的storage一致的位置做的替换
    //因为head代表的这个队列一定有同一个storage
    if(prev != null)
  ...
  public boolean isComplete() {
	  //不是BlockInfoContiguousUnderConstruction对象一定返回true
  ...
  public BlockInfoContiguousUnderConstruction convertToBlockUnderConstruction(
    ...
    if(isComplete()) {//构造BlockInfoContiguousUnderConstruction对象
    ...
    //对象以及是一个BlockInfoContiguousUnderConstruction对象了
    BlockInfoContiguousUnderConstruction ucBlock =
