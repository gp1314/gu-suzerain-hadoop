...
class BPServiceActor implements Runnable {
  ...
  //管理当前BPServiceActor对象的BPOfferService对象
  ...
  //当前的工作线程，是BPServiceActor主逻辑执行线程
  Thread bpThread;
  //向namenode发送RPC请求的代理
  ...
  // to elapse.
  //IBR标志，只要该标志置位，就会立即发送增量块汇报，不用等待超时后再发送
  private volatile boolean sendImmediateIBR = false;
  //标志BPServiceActor的启动状态
  ...
  private void checkNNVersion(NamespaceInfo nsInfo)
    ...
    //如果namenode的版本小于最小版本配置，默认为2.1.0-beta
      ...
      if (!success) {
        synchronized (pendingIncrementalBRperStorage) {
          for (StorageReceivedDeletedBlocks report : reports) {
        	...
        	//如果发送失败，则将reports中的信息重新放入队列
        	//设置sendImmediateIBR标志，立即重新发送
  ...
  void notifyNamenodeBlock(ReceivedDeletedBlockInfo bInfo,
    ...
    synchronized (pendingIncrementalBRperStorage) {
      ...
      //sendImmediateIBR置位表示需要立即发送增量块汇报给namenode
      if (now) {
  ...
  List<DatanodeCommand> blockReport() throws IOException {
    ...
    // a FINALIZED one.
    //先向namenode汇报增量块信息，避免namenode和datanode元数据不同步
  ...
  private String formatThreadName() {
	//遍历配置中的存储目录，构造成StorageLocation对象
	//其中包含了存储目录的存储类型
  ...
  private void offerService() throws Exception {
    ...
    while (shouldRun()) {
      try {
        ...
        //如果是注册信息错误，或者datanode不合法，或者Version信息错误，直接停止线程运行
  ...
  void register(NamespaceInfo nsInfo) throws IOException {
    ...
    //确认namespaceid clusterid与另一个actor获取的是否一致
    ...
    try {
      while (true) {
        ...
        try {
          ...
          //握手注册异常，将运行状态改为INIT_FAILED
          runningState = RunningState.INIT_FAILED;
          if (shouldRetryInit()) {
            // Retry until all namenode's of BPOS failed initialization
        	//重试直到BPOfferService停止初始化操作
      ...
      //线程的主循环，循环调用offerService方法向namenode发送心跳，快汇报，增量块汇报以及缓存块汇报
  ...
  public void bpThreadEnqueue(BPServiceActorAction action) {
	//加入bpThreadQueue队列中，等主工作线程处理队列中的action
