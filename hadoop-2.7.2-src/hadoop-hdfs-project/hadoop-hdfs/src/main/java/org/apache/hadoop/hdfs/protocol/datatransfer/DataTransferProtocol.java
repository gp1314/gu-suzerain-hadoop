...
public interface DataTransferProtocol {
   ...
   */
  //将指定的数据块复制到另一个datanode上
  //数据块复制操作是数据流管道中的数据节点出现故障，需要用新的数据节点替换异常的数据节点时
  //dfsclient调用这个方法将数据流管道中正常数据节点上已经写入的数据块复制到新添加的数据节点上
  ...
  //写成功后通知namenode，并删除源datanode上的数据块
  //该操作主要用于数据块平衡操作的场景
