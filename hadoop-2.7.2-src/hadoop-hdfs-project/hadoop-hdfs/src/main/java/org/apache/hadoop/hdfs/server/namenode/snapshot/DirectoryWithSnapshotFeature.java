...
public class DirectoryWithSnapshotFeature implements INode.Feature {
   ...
   */
  //从parent目录中删除child，调用者必须确保parent在最新的快照中
  public boolean removeChild(INodeDirectory parent, INode child,
    ...
    // any snapshot.
	// 如果一个目录没有被重命名，如果isInLatestSnapshot返回false，则该目录不在最新快照中，因此我们不需要在任何快照中记录已删除的子目录
	// 对于在前一个版本的快照中的被删除或重命名的目录，isInLatestSnapshot会返回true（如果在前一个版本中已经被删除了，isInLatestSnapshot还是会返回false）
	// 因此如果isInLatestSnapshot返回false，则该目录不在任何快照中（不在当前目录树中也不在以前版本的源目录树种）
	// 因此不用在任何快照中记录被删除的child
    ChildrenDiff diff = diffs.checkAndAddLatestSnapshotDiff(latestSnapshotId,
        parent).diff;
    UndoInfo<INode> undoInfo = diff.delete(child);
    //在parent的children列表中查找该child
    final boolean removed = parent.removeChild(child);
    if (!removed && undoInfo != null) {
      // remove failed, undo
    	//没找到该clild需要回退diff的操作（为什么不先判断parent里面有没有，再操作diff）
