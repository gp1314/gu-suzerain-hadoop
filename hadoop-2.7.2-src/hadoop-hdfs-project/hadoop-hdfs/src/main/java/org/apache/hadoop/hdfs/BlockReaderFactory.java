...
public class BlockReaderFactory implements ShortCircuitReplicaCreator {
  ...
  public BlockReader build() throws IOException {
    ...
    if (conf.shortCircuitLocalReads && allowShortCircuitLocalReads) {
      //老版本的BlockReaderLocal，当客户端与datanode在同一台机器上时
      //客户端直接从磁盘读取数据，老版本的实现要求客户端获取datanode数据目录的权限，这可能引起安全问题
  ...
  private BlockReader getBlockReaderLocal() throws InvalidToken {
    ...
    //获取client的ShortCircuitReplicaInfo对象的缓存对象
    ShortCircuitCache cache = clientContext.getShortCircuitCache();
    ExtendedBlockId key = new ExtendedBlockId(block.getBlockId(), block.getBlockPoolId());
    //读取缓存中的ShortCircuitReplicaInfo对象，如果缓存没有就构造一个
    ...
    //检查ShortCircuitReplica是否存在
    //ShortCircuitReplica保存了用来执行短路读取的文件描述符，client和datanode共享内存中记录当前副本
    //信息的slot对象
