...
public class DatanodeDescriptor extends DatanodeInfo {
  ...
  //datanode中关于storageid->DatanodeStorageInfo的表
   ...
   */
  //发送最后一批缓存指令的时间，以毫秒为单位。
  private long lastCachingDirectiveSentTimeMs;

  // isAlive == heartbeats.contains(this)
  // This is an optimization, because contains takes O(n) time on Arraylist
  public boolean isAlive = false;//表示当前dtanode是否有效
  ...
  /*
   * 系统管理员可用通过dfsadmin -setBalanacerBandwidth更改balancer带宽
   * 新设置的带宽值将会赋值给bandwidth字段，并通过心跳下发到指定datanode
   * 当下发完成后果bandwith将会被设置为0
   * */
  private long bandwidth;

  /** A queue of blocks to be replicated by this datanode */
  //下发复制命令时，携带的数据块和目标数据节点信息
  private final BlockQueue<BlockTargetPair> replicateBlocks = new BlockQueue<BlockTargetPair>();
  /** A queue of blocks to be recovered by this datanode */
  //下发数据块恢复指令，携带恢复操作的副本信息
   ...
   */
  //估计datanode的负载，在写文件操作分配数据块或进行数据块复制时，
  //根据节点负载信息，选择比较空闲的节点作为目标节点
  ...
  boolean removeBlock(BlockInfoContiguous b) {
    ...
    // 删除datanode中的blockList队列中的blockinfo,以及blockinfo中的DatanodeStorageInfo
    if (s != null) {
  ...
  public void updateHeartbeatState(StorageReport[] reports, long cacheCapacity,
    ...
    //除去正常心跳的storage，剩下的就是损坏的storage
      if (checkFailedStorages) {
