...
public class LeaseManager {
  ...
  synchronized Lease addLease(String holder, String src) {
    ...
    if (lease == null) {
      ...
      leases.put(holder, lease);//添加持有者和租约关系
      sortedLeases.add(lease);//添加租约
    } else {
      renewLease(lease);//刷新租约更新时间
    }
    sortedLeasesByPath.put(src, lease);//添加文件与租约关系
    lease.paths.add(src);
    return lease;
  }
  ...
  /*
   * 租约类管理单个客户端持有的所有锁。
   * 每个客户端都持有的租约其时间戳周期性的更新
   * 如果一个客户端口挂死允许其释放租约，文件对应的所有相应的锁都可以释放。
   * */
  class Lease implements Comparable<Lease> {
    ...
    //更新客户端lastUpdate最近更新时间
    private void renew() {
      this.lastUpdate = monotonicNow();
    }

    /** @return true if the Hard Limit Timer has expired */
    //判断当前租约是否超出了硬限制，硬限制是用于考虑文件关闭异常时，强制回收租约的时间，默认60s,不可配置
    //LeaseManager中的Monitor用于定期检查租约更新情况，当超过硬限制，则会触发租约恢复机制
    public boolean expiredHardLimit() {
      return monotonicNow() - lastUpdate > hardLimit;
    }

    /** @return true if the Soft Limit Timer has expired */
    //判断当前租约是否超出了软限制，软限制是写文件规定的租约超时时间，默认60s，不可配置
