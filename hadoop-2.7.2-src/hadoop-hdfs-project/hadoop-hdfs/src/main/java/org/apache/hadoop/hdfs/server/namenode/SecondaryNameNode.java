...
public class SecondaryNameNode implements Runnable,
  ...
  static boolean downloadCheckpointFiles(
    ...
    //检查上一个checkpoint的事务id是否与editlog文件的开始事务id是否连续
  ...
  public boolean doCheckpoint() throws IOException {
    ...
    //触发activenamenode重置editlog,新建一个edit.new文件记录执行日志
  ...
  static void doMerge(
    ...
    dstStorage.setStorageInfo(sig);
    //如果是启动的第一个checkpoint首先要加载fsimage文件
    if (loadImage) {
    ...
    //将新的fsimage文件持久化到磁盘，并删除过期的fsimage、editlog文件
