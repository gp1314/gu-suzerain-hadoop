...
public class EditLogFileOutputStream extends EditLogOutputStream {
  ...
  private File file;//输出流对应的ediltlog文件
  ...
  private EditsDoubleBuffer doubleBuf;//一个具有两块缓冲的缓冲区
  static final ByteBuffer fill = ByteBuffer.allocateDirect(MIN_PREALLOCATION_LENGTH);//用来扩充editlog文件大小的数据块
  ...
  private void preallocate() throws IOException {
    ...
    while (need > 0) {
      fill.position(0);
      //将填充缓冲区写入通道，但不改变position，就起到扩充通道的作用
