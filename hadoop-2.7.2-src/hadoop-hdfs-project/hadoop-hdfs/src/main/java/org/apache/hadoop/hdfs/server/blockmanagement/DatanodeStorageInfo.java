...
public class DatanodeStorageInfo {
  ...
  private final DatanodeDescriptor dn;//当前storage所在的datanode的DatanodeDescriptor
  private final String storageID;//该storage在集群内唯一的表示符
  private StorageType storageType;//描述当前storage是什么类型，如磁盘，闪存
  private State state;//当前storage的状态信息

  private long capacity;//容量
  private long dfsUsed;//使用量
  private volatile long remaining;//剩余容量
  private long blockPoolUsed;//块池使用量
  //记录当前storage上保存的数据块副本链表的头结点
  private volatile BlockInfoContiguous blockList = null;
  private int numBlocks = 0;//保存的副本数量
  ...
  public AddBlockResult addBlock(BlockInfoContiguous b) {
    ...
    if (otherStorage != null) {//已经存在该block与datanode的映射关系
      if (otherStorage != this) {//该block不是当前datanode的存储类型
    ...
    //还没有建立映射关系，将该datanode信息存入BlockInfo队列中的最后一个位置
    b.addStorage(this);
    //将b更新为该datanode的blockList的头结点
