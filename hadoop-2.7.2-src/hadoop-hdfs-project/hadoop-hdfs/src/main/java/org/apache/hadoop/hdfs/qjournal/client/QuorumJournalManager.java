...
public class QuorumJournalManager implements JournalManager {
  ...
  QuorumJournalManager(Configuration conf,
    ...
    this.uri = uri;
    this.nsInfo = nsInfo;
    //构造AsyncLoggerSet对象，维护集群所有JouranlNode之间的连接
  ...
  Map<AsyncLogger, NewEpochResponseProto> createNewUniqueEpoch()
    ...
    //将最大的epoch number值加一更新到所有JN上，并作为这个QJM的epoch number
  ...
  private void recoverUnclosedSegment(long segmentTxId) throws IOException {
    ...
    //通过SegmentRecoveryComparator.INSTANCE比较出JN的状态
    //选择出一个最好的editlog文件作为源文件
    ...
    if (bestResponse.hasAcceptedInEpoch()) {
      ...
      // 没有segment的装状态有可能：
      // 	有JN1-JN3个节点，在写入segment 101到JN1成功，还有写到JN2、JN3就失败了
      //	然后进行更新epoch number操作，执行恢复步骤，在prepare之前JN1又崩溃了
    ...
    //筛选出的editlog文件的url，其他JN可以通过这个url读取源editlog文件
    URL syncFromUrl = bestLogger.buildURLToFetchLogs(segmentTxId);
    //向所有JN发送accept请求，JN会执行
    //	通过选定节点的editlog
    //	持久化恢复元数据，就是记录恢复操作是的editlog文件状态以及epoch number等信息
    ...
    // finalize.
    // editlog文件已经同步，可以执行finalized操作了
  ...
  public EditLogOutputStream startLogSegment(long txId, int layoutVersion)
    ...
    //向每个AsyncLogger提交startLogSegment任务
    QuorumCall<AsyncLogger, Void> q = loggers.startLogSegment(txId,
        layoutVersion);
    //等待返回的结果，如果失败超过阈值，抛出异常
  ...
  public void recoverUnfinalizedSegments() throws IOException {
    ...
    //更新epoch number到JN让原来的active节点失效
  ...
  public void selectInputStreams(Collection<EditLogInputStream> streams,
	...
	//向所有JN节点发送读取请求，获取RemoteEditLogManifest
    ...
    for (Map.Entry<AsyncLogger, RemoteEditLogManifest> e : resps.entrySet()) {
      ...
      //为JN上保存的每个editlog段落文件构造EditLogInputStream对象
    ...
    //将所有EditLogFileInputStream封装在一个RedundantEditLogInputStream
